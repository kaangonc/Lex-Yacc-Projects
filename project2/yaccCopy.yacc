%{
	#include<stdio.h>
%}
%token MOVE_FUNC GRAB_FUNC RELEASE_FUNC TURN_FUNC READ_FUNC SEND_FUNC RECEIVE_FUNC INT_TYPE FLOAT_TYPE STRING_TYPE BOOLEAN_TYPE VOID_TYPE CONST_TYPE PLUS MINUS ASTERISK SLASH MOD IF ELSE WHILE FOR END START COMMA LEFT_P RIGHT_P LEFT_SQUARE_P RIGHT_SQUARE_P SCOPE_BEGIN SCOPE_END OR EQUAL IS_EQUAL IS_NOT_EQUAL LESS_OR_EQ GREATER_OR_EQ LESS_THAN GREATER_THAN AND NOT RETURN BOOLEAN INTEGER FLOAT COMMENT VARIABLE FUNCTION_NAME CONST ASK PRINT SEMICOLON
%% 
...
non_block_stmt : return_stmt | assignment_stmt | var_declaration | function_call_stmt | scan_statement | print_statement | constant_declaration ;
...
equality_expression : relational_expression| relational_expression equality_operator relational_expression | additive_expression equality_operator additive_expression ;
 
relational_expression : additive_expression relational_operator   
additive_expression | boolean_expression ;
 
boolean_expression : BOOLEAN | LEFT_P conditional_expression RIGHT_P ;
...
primary_expression : VARIABLE | primitive_type |  function_call_stmt | LEFT_P additive_expression RIGHT_P ;
...
move_func : MOVE_FUNC LEFT_P RIGHT_P ;
...
for_stmt : FOR LEFT_P VARIABLE SEMICOLON conditional_expression SEMICOLON assignment_stmt RIGHT_P body ;
...
type_name : INT_TYPE | FLOAT_TYPE | BOOLEAN_TYPE ;
